...
def get_AuthToken(cr_id, operator_url, app_config):
    ...
    print(cr_id)
    token = get("{}/api/1.3/cr/auth_token/{}".format(operator_url, cr_id))  # TODO Get api path from some config?
    ...
    #req = post("http://service_components:7000/api/1.3/sink_flow/dc", json=data)
    # req = get("http://service_components:7000/api/1.3/sink_flow/"
